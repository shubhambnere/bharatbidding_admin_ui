...
const ProductMoreMenu = ({ record, handleDelete }) => {
  ...
  const handleEditClick = () => {
    const url = `/product-profile`;
    // const url = `/${kebabCase ('Product-ABC')}/?id=${'12345ismyid'}`;
    history.push(url);
  };
  return (
    <>
     {/* <Link  to={`/product-profile/${kebabCase ('Product-ABC')}/?id=${'12345ismyid'}`}>    */}
  <Button
  // component={RouterLink}  to={`/${kebabCase ('Product-ABC')}/?id=${'12345ismyid'}`}
    size="small"
  ...
  >
    ...
    <Link   to={`/product-profile/?id=${'12345ismyid'}`}>
